...
function videoChange(AvideojsName,BvideojsName,CvideojsName,DvideojsName, AvideoName,BvideoName,CvideoName,DvideoName, local){
	...
	readTextFile(txtName("aaCCTV", ".txt",1,local), function(result) {
		aaCCTV = stringProcess(result);
	});
	readTextFile(txtName("bbCCTV", ".txt",1,local), function(result) {
		bbCCTV = stringProcess(result);
	});
	readTextFile(txtName("ccCCTV", ".txt",1,local), function(result) {
		ccCCTV = stringProcess(result);
	});
	readTextFile(txtName("ddCCTV", ".txt",1,local), function(result) {
	...
	.done(function(json){ 
		...
		console.log("videopath : " + local+"/resources/video/"+AvideoName+(json.num).toString()+".mp4");
		AvideojsName.src({
			type : 'video/mp4',
			src : local+"/resources/video/"+AvideoName+(json.num).toString()+".mp4"
		});
		BvideojsName.src({
			type : 'video/mp4',
			src : local+"/resources/video/"+BvideoName+(json.num).toString()+".mp4"
		});
		CvideojsName.src({
			type : 'video/mp4',
			src : local+"/resources/video/"+CvideoName+(json.num).toString()+".mp4"
		});
		DvideojsName.src({
			type : 'video/mp4',
			src : local+"/resources/video/"+DvideoName+(json.num).toString()+".mp4"
		});
